...
=pod
getClusterInstanceIds.pl us-east-2 yuting-oxford-university-hpcc-cluster-5 10.0.0.145 10.0.0.37 &> yuting-cluster-instance-ids.log
getClusterInstanceIds.pl $region $stackname 10.0.0.102 10.0.0.130 &> cluster-instance-ids.log
...
#print "DEBUG: Entering getClusterInstanceIds.pl region=\"$region\", stackname=\"$stackname\", \@private_ip=(",join(",",@private_ip),")\n";

$DesiredInfo[0]='InstanceId';
$DesiredInfo[1]='PrivateIp';
...
@InstanceIds = grep(s/\s*$re//, @InstanceInfo);
print join("\n",@InstanceIds),"\n";

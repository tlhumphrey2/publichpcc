...
#print "DEBUG: Entering getClusterInstanceInfo. region=\"$region\", stackname=\"$stackname\". Size of \@DesiredInfo is ",scalar(@DesiredInfo),".\n";
  ...
  $qElement{'KeyName'} = 'KeyName';
  $qElement{'sgnames'} = 'SecurityGroups[*].[GroupId]';
  ...
  $qElement{'asg'} = "[Tags[?Key=='aws:autoscaling:groupName'].Value][0][0]";
  ...
  push @qElement, $qElement{'sgnames'};

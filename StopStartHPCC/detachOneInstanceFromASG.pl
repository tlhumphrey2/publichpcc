...
if ( $asgname eq '' ){
  ...
  if ( $asgname =~ /^\s*$/s ){
    ...
    $asgname =~ s/:.+$//;
    my $dt=formatDateTimeString(); print("$dt (1) aws autoscaling detach-instances --instance-ids $instance_id --auto-scaling-group-name $asgname --should-decrement-desired-capacity --region $region\n");
    my $rc=`aws autoscaling detach-instances --instance-ids $instance_id --auto-scaling-group-name $asgname --should-decrement-desired-capacity --region $region`;
    print "$dt $rc";
  }
}else{
  my $dt=formatDateTimeString(); print("$dt (2) aws autoscaling detach-instances --instance-ids $instance_id --auto-scaling-group-name $asgname --should-decrement-desired-capacity --region $region\n");
  my $rc=`aws autoscaling detach-instances --instance-ids $instance_id --auto-scaling-group-name $asgnam --should-decrement-desired-capacity --region $region`;
  print "$dt $rc";
}
#========================================================================================
sub waitUntilAlive{

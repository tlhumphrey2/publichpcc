...
=pod
sudo ~/desprayAllLogicalFilesToS3.pl mhpcc-ca-central-1-xrt-164 thor
=cut
...
$cp2s3_logname = "desprayAllLogicalFilesToS3.log";
openLog($cp2s3_logname);

printLog($cp2s3_logname,"Entering desprayAllLogicalFilesToS3.pl s3bucketpath=\"$s3bucketpath\".\n");
...
printLog($cp2s3_logname,"In desprayAllLogicalFilesToS3.pl. master_pip=\"$master_pip\"\n");
...
# Make a folder for metadata files
  ...
  foreach (@FilesOnThor){
     ...
     my $filename = $_;
     $filename =~ s/::/\//g;
     my $dstdir = "$s3bucketpath/despray";
  ...
  printLog($cp2s3_logname,"Completed getting metadata for files.\n");
   ...
   printLog($cp2s3_logname,"In desprayAllLogicalFilesToS3.pl. There are no files on the thor.\n");
}
#=============================================
sub putInFile{
my ($ecl, $outfile)=@_;

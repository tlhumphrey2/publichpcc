...
$stdout = *STDOUT;
$cp2s3_logname='listLogicalFilesWithClusterNameAndSizes.log';
openLog($cp2s3_logname);

printLog($cp2s3_logname,"Entering listLogicalFilesWithClusterNamesAndSizes.pl stdout=\"$stdout\".\n");

($master_pip, @slave_pip)=thor_nodes_ips();
printLog($cp2s3_logname,"In listLogicalFilesWithClusterNamesAndSizes.pl. master_pip=\"$master_pip\"\n");
...
# Make a folder for metadata files
  ...
  foreach (@FilesOnCluster){
     ...
     push @FilesAndClusters, outputFilenameClusterAndSize($xmlfile);
  }
  ...
  *STDOUT = $stdout;
  print join("\n",@FilesAndClusters),"\n";
   ...
   printLog($cp2s3_logname,"In listLogicalFilesWithClusterNamesAndSizes.pl. There are no files on the thor.\n");
}
#===================================================
sub outputFilenameClusterAndSize{
my ($xmlfile)=@_;
  ...
  local $_ = `egrep "size=" $xmlfile`;
  if ( /\bsize="(\d+)"/ ){
      $fac = "$fac,$1";
  }
  printLog($cp2s3_logname,"Leaving outputFilenameClusterAndSize. xmlfile=\"$xmlfile\", filename=\"$filename\", fac=\"$fac\".\n");
return $fac;
}

...
if ( scalar(@ARGV)>0 ){
  ...
  $private_ips=shift @ARGV;
}

$mountDisks=1;

# Get all private_ips
print "Get all private ips from $private_ips.\n";
...
for( my $i=$#private_ips; $i >= 0; $i--){ 
  ...
  if ( $mountDisks  && ! $EBSVolumesMountedByFstab && ($stackname !~ /mhpcc-/) ){
    if ( $ephemeral ){
...
$rc=`for x in \$(cat $private_ips);do ssh -i $pem -t $sshuser@\$x "echo $x;df -BG $mountpoint";done`;
